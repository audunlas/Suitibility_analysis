...
def main():
    if len(sys.argv) != 4:
        print("Usage: python report.py <model_pkl> <historic_data_csv> <out_file.pdf>")
        sys.exit(1)

    model_path, historic_data_path, out_file = sys.argv[1], sys.argv[2], sys.argv[3]

    model = SuitabilityModel.from_dict(joblib.load(model_path))
